...
def plot_pre_traj(agent_list,obstacle_list,show,episodes):
    ...
    if show:
        ...
        for i in range(len(agent_list)):
            ...
            circle=Circle(xy=agent_list[i].pre_traj[0],radius=SET.radius,fc=color[i],ec='k',lw=1.0 )
...
def plot_position(agent_list,obstacle_list):
    ...
    for j in range(len(agent_list[0].position)):
        for i in range(SET.Num):
            ...
            circle=Circle(xy=agent_list[i].position[j],radius=SET.radius,fc=color[i],ec='k' )
