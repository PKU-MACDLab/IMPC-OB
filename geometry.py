...
def detect_polygon_polygon_collision(poly1,poly2):
    ...
    for line1 in poly1.line_list:
        for line2 in poly2.line_list:
            if detect_line_line_collision(line1,line2):
                return True

    # Judging whether one polygon is included in another one. 

    v1=poly1.vertex_list[0]
    v2=poly2.vertex_list[0]

    if not poly1.type == 'line':
        if detect_point_in_ob(poly1,v2):
            return True

    if not poly2.type == 'line':
        if detect_point_in_ob(poly2,v1):
            return True

    return False


def detect_polygon_line_collision(poly,line1,flag=True):

    for line2 in poly.line_list:
        if detect_line_line_collision(line2,line1):
            return True

    if flag:
        v=line1.vertex_list[0]

        if detect_point_in_ob(poly,v):
            return True
